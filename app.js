...
async function init(){bind();setupAtmosphere();updateMusicButton();try{const response=await fetch("corpus.json");if(!response.ok)throw new Error(response.status);corpus=await response.json();index=Math.max(0,Math.min(Number(state.current)||0,99));renderCanto(index);$("#roleGate").hidden=Boolean(state.role)}catch{$("#promptText").textContent="档案未能载入";$("#worldPrompt").disabled=true}}
init();
